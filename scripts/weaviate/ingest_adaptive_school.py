#!/usr/bin/env python3
"""
Script to ingest Adaptive School documents into Weaviate with hybrid tag generation.
...
3. Uses LangChain text splitter to chunk documents
4. Ingests the chunks into the AdaptiveSchools Weaviate collection
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("ingest_adaptive_school.log")
    ]
)
...
def prepare_document_for_pipeline(document):
    """Prepare a document for processing by the document pipeline."""
    # Convert tags list to a comma-separated string
    tags_string = ", ".join(document['tags']) if document['tags'] else ""
    
    # Create a document dictionary in the format expected by the pipeline
    pipeline_document = {
    ...
    if document.get('chapterNumber') is not None:
        pipeline_document['metadata']['chapterNumber'] = document['chapterNumber']
...
def process_and_ingest_all_documents(args):
    ...
    # Configure the pipeline for Weaviate ingestion
    pipeline_config = {
        'pipeline_type': 'weaviate',
        'collection_name': 'AdaptiveSchools',
        'chunker_config': {
            'chunk_size': args.chunk_size,  # Use the chunk size from command line args
            'chunk_overlap': args.chunk_overlap  # Use the chunk overlap from command line args
        },
        'weaviate_enabled': True
    }
    
    # Create the pipeline with the Weaviate client
    pipeline = DocumentPipeline(config=pipeline_config, weaviate_client=client)
    ...
    for file_path in input_dir.glob("*.md"):
        ...
        try:
            ...
            # Prepare the document for the pipeline
            pipeline_document = prepare_document_for_pipeline(document)
            
            # Process the document with the pipeline (which will handle chunking and Weaviate upload)
            try:
                result = pipeline.process_document(pipeline_document)
                ...
                stats["error_count"] += 1
                logger.error(f"Error processing document with pipeline: {e}")
